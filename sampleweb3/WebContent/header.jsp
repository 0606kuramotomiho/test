...
function goLogoutAction(){
	document.getElmentById("form").action="LogoutAction";
}
...
<s:form id="form" name="form">
	<s:if test='#session.containsKey("mCategoryDtoList")'>
		...
		<li><s:textfield name="keywords" class="txt-keywords" placeholder="SEARCH" /></li>
	...
	<s:else>
		<li><s:submit value="ログイン" class="submit_btn" onlick="goLoginAction();"/></li>
	</s:else>
	...
	<s:if test="#session.logined==1">
		<li><s:submit value="マイページ" class="submit_btn" nclick="goMyPageAction();"/></li>
	</s:if>
</s:form>
</ul>
</div>
